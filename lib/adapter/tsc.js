...
class Tsc {
  ...
  text ({ text, x = 3, offsetTop = 0, align = 'left', wide = 1, high = 1, noWrap, hasHigh }) {
    if (this.tempX) {
      x = this.tempX
    }
  ...
  tableCustomExt (data) {
    let x = 3, hasHigh
    tableCombine(this.width, data, ({ text, size, verticalAlign, notLast }) => {
      const [wide, high] = size
      !hasHigh && (hasHigh = high === 2)
      const offsetTop = verticalAlign === 'bottom' && hasHigh && high === 1 ? 20 : 0
      this.text({ text, x, offsetTop, wide, high, noWrap: notLast, hasHigh })
      if (notLast) {
      ...
      Image.load(url, 'image/png', image => {
        if (image instanceof Image) {
          ...
          this.rasterArray.map((raster, i) => {
            ...
            this.buffer.write('\r\n')
            this.buffer.write(`REVERSE ${x},${y},${width*8},${height}\r\n`)
          })
          cb && cb(null, this)
          resolve()
        } else {
