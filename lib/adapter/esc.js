  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (cellWidth < textLenghth || (obj.chunks && obj.chunks.length)) {
      ...
      switch (overflow) {
        ...
        case 'HIDDEN':
          tooLong = false
          obj.text = sliceText(obj.text, cellWidth / 2)[0]
          break
        case 'WRAP':
        default:
          obj.chunks = obj.chunks || wordWrap(obj.text, cellWidth / 2)
    ...
    if (trueText.trim().length > 0) {
      ...
      if (color === 'reverse-block' && reg?.test(text)) {
        ...
        strs.map((str, j) => {
          notLast = !notLast && j < strs.length - 1
          if (str === trueText) {
            this.style('b')
            this.setReverseColors(true)
            this.text(trueText, null, notLast)
          } else if (str !== '') {
            this.setReverseColors(false)
            this.text(str, null, notLast)
          }
        })
