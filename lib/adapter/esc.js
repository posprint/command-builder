...
escpos.Printer.prototype.tableCustomExt = function (data, encoding) {
    tableCombine(this.width, data, ({ text, trueText, notLast, size, color, reverseRange }) => {
      const _color = (color === 1 || color?.indexOf?.('red') > -1) ? 1 : 0
      this.color(_color)
      size && this.size(...size)
      if (trueText.trim().length > 0) {
        ...
        if (color?.indexOf?.('reverse-block') > -1 && reg?.test(text)) {
        ...
        } else if (color?.indexOf?.('reverse') > -1){
          this.style('b')
          this.setReverseColors(true)
          ...
          if (reverseRange && Array.isArray(reverseRange)) {
            ...
            if (start !== 0) {
              this.text(text.substr(0, start), null, notLast)
            }
            this.style('b')
            this.setReverseColors(true)
            this.text(text.substr(start, end), null, true)
            this.style('Normal')
            this.setReverseColors(false)
            this.text(text.substr(end, text.length), null, notLast)
          } else {
            this.text(text, null, notLast)
          }
        }
