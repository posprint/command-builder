...
class Commander {
  ...
  get width () {
    if (this.isEsc) {
      return escPaperWidthConvert(this.paperSize[0].toString())
    } else if (this.isTsc) {
      return this.paperSize[0]
    } else if (this.isPax) {
      return 31
    }
  }
  ...
  get wordWidth () {
    if (this.isEsc || this.isPax) {
      ...
      return tscPaperWidthConvert(this.paperSize[0].toString())
    }
  }
  ...
  send (cmd, ...args) {
    this.adapter[cmd] && this.adapter[cmd](...args)
  }
