...
const tableCombine = (width, data, printFun, type) => {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    const verticalAlign = obj['vertical-align']
    const fontSize = obj['font-size'] || 'normal'
    const size = converter(type || 'esc', 'font-size', fontSize) || [1,1]
    let ratio = size[0]
    // if (ratio === 2 && type === 'esc' && i === 0 && data.length > 1 && !obj.text.trim()) {
    //   ratio = 1
    //   size = [1,1]
    // }
    ...
    if (/CENTER/i.test(obj.align)) {
      ...
      // console.log('|'+text+'|', cellWidth, calWords(trueText), spaces)
    }
    ...
    // console.log('|' + text + '|', 'calWords ' + calWords(text), 'cellWidth ' + cellWidth, 'textLenghth ' + textLenghth, 'width ' + width, notLast)
    const color = (obj['color'] && converter(type || 'esc', 'color', obj['color'])) || obj['color']
    const reverseRange = obj['reverse-range']
    if (reverseRange) {
      obj['reverse-range'] = null
    }
    printFun({ text, trueText, notLast, size, align: obj.align, verticalAlign, color, reverseRange })
  }
